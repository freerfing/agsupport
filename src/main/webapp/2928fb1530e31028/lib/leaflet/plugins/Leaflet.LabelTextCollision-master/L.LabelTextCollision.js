L.LabelTextCollision = L.Canvas.extend({
    ...
    _text: function (ctx, layer) {
        if (layer.options.text != undefined) {
            ...
            var textArr=layer.options.text;
            var offsetX = 2;
            var offsetY = -(fontSize*textArr.length-10);
            var length=0;
            var maxIndex=0;
            for(var i in textArr){
                var tempLength=textArr[i].length;
            ...
            var textWidth = (ctx.measureText(textArr[maxIndex]).width) + p.x;
            var textHeight =p.y + offsetY + fontSize*textArr.length;
            ...
            var textColor = layer.options.textColor;
            for(var i in textArr){
                //描边
                // ctx.strokeText(textArr[i], p.x + offsetX, p.y + offsetY+fontSize*i);
                var x=p.x + offsetX;
                var y=p.y + offsetY+fontSize*i;
                // 矩形
                ctx.fillStyle = "white";
                ctx.fillRect(x-1,y-fontSize+1,ctx.measureText(textArr[i]).width+2,fontSize+2);
                // 填充字
                ctx.fillStyle = "blue";

                if (textColor == undefined) {
                    ctx.fillStyle = "blue";
                } else if(Object.prototype.toString.call(textColor)=='[object Array]'&&textColor.length>=textArr.length){//如果是数组，并且颜色的数组的个数要与分行的内容相一致
                    var color = textColor[i];
                    if(null!=color){
                        ctx.fillStyle = color;
                    }

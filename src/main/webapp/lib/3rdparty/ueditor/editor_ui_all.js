...
(function (){
    ...
    var utils = baidu.editor.utils = /**@lends baidu.editor.utils.prototype*/{
        ...
        /**
         * 将css样式转换为dom的形式。如font-size -> fontSize
         * @public
         * @function
         * @param {String} str      需要转义的样式
         * @returns {String}        转义后的样式
         */
...
(function () {
    ...
    /**
     * 获得对象所拥有监听类型的所有监听方法，没有则创建
     * @public
     * @function
     * @param {Object} obj  对象
     * @param {String} type 事件类型
     * @param {Boolean} force  没有的话是否创建
     * @param {Boolean} force  没有的话是否创建
     * @returns {Array} 鉴定者的数组
     */
...
//                }
         ...
         * @return   {boolean}   返回元素A和元素B的style属性是否一致
         */
...
//                        }
         ...
         * 检查是否是inline的套用的空节点
         * @param   {Node}    node      节点
         * @return {Boolean}  是否是inline的套用的空节点
         * @example
         * &lt;b&gt;&lt;i&gt;&lt;/i&gt;&lt;/b&gt; //true
         * <b><i></i><u></u></b> true
         ...
         * @function
         * @param {Element} element      元素
         * @param {Array} attrNames      要删除的属性名称
         */
         ...
         * @function
         * @param {Node} node      节点
         * @param {Array} attrNames     要添加的属性名称
         */
...
(function() {
    ...
    Range.prototype = {
         ...
         * @return {Fragment}    frag|null返回选中内容的文本片段或者空
         */
         ...
         * @param {Boolean}    true为闭合到节点后，false为闭合到节点前
         * @return  {Range}    返回range对象
         */
         ...
         * @param {Boolean}   serialize    true：创建start为元素的书签
         * @param  {Boolean}   same        true：创建id为_baidu_bookmark_start_的书签
        ...
        _setup: function ( doc ) {
           ...
           var html = ( ie ? '' : '<!DOCTYPE html>' ) +
            ...
            if(this.options.initialContent){
                if(this.options.autoClearinitialContent)
...
//                }
        ...
        setDefaultContent : function(){
             function clear(type,flag){
                var me = this;
                if(type == 'beforeselectionchange' && flag)return;
                if(me.document.getElementById('initContent')){
                    ...
                    me.removeListener('beforeselectionchange',clear);
                    me.removeListener('beforeexeccommand',clear);
                    setTimeout(function(){
                        me._selectionChange();
                    },0)
                }
            ...
            return function (cont){
                ...
                me.removeListener('beforeselectionchange',clear);
                me.removeListener('beforeexeccommand',clear);
                me.addListener('beforeselectionchange',clear);
                me.addListener('beforeexeccommand',clear);
            }
...
(function() {
    ...
    baidu.editor.plugins['pagebreak'] = function() {
        ...
        me.commands['pagebreak'] = {
            execCommand:function(){
                var range = me.selection.getRange(),
                    pageBreakTag = me.options.pageBreakTag;
                range.collapse(true);
        ...
        me.addListener('ready',function(){
            ...
            domUtils.on(me.body, browser.ie ? 'beforepaste' : 'paste',function(e){
                 getClipboardData.call( me, function( div ) {
                     filter(div);
                     if ( modify_num.flag ){
                        editor.ui.showToolbarMsg( "编辑器将过滤掉您粘贴内容中的不支持格式！" );
                        modify_num.flag = "";
                     }
                    } );
 ...
 (function(){
    ...
    function checkHasUI(){
        try{
            ...
            alert('autoflow插件功能依赖于UEditor UI\nautoflow定义位置: _src/plugins/autoheight/autoheight.js');

            throw({
                name: '未包含UI文件',
                message: 'autoflow功能依赖于UEditor UI。autoflow定义位置: _src/plugins/autoheight/autoheight.js'
            });
        }
        return 1;
    }
...
(function(){
    ...
    function CLASS_HIGHLIGHT(code,syntax){
        ...
        this.highlight = function() {
            ...
            htmlTxt[htmlTxt.length] = '<span style=" text-align: right;padding:2px 10px  0;border-right:5px solid #ccc;margin:0 10px 0 0;color:#000;">' + line_num + '.' + addSpace(line_num) + '</span>';
            //按分割字，分块显示
            for (var i=0; i <=word_index; i++){
                ...
                //处理换行
                    ...
                    line_num++;
                    htmlTxt[htmlTxt.length] = ('<br/><span style="text-align: right;padding:2px 10px  0;height:14px;border-right:5px solid #ccc;margin:0 10px 0 0;color:#000;">' + line_num + '.' + addSpace(line_num) + '</span>');
        ...
        me.commands['highlightcode'] = {
            execCommand: function (cmdName, code, syntax) {
                if(code && syntax){
                    ...
                    highlight.highlight();
                    me.execCommand('inserthtml', "<pre _syntax='"+syntax+"' style='background-color: #F8F8F8;border: 1px solid #CCCCCC;padding:10px 10px;margin:5px;word-wrap:normal;'>"+highlight._codetxt+"</pre>");
                    ...
                    if(domUtils.getPosition(start,end) == 0){
                        start.innerHTML = "";
                        domUtils.remove(start);
                    }else{
                        if(start){
            ...
            queryCommandState: function(cmdName){
                 var range = this.selection.getRange();
                return (domUtils.findParentByTagName(range.startContainer, 'pre', true) || domUtils.findParentByTagName(range.endContainer, 'pre', true)) ? 1 : -1;
            }
        };
        me.addListener("beforegetcontent",function(){
            for(var i=0,pr,pres = domUtils.getElementsByTagName(me.document,"pre");pr=pres[i++];){

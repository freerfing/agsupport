 ...
 * 河道水文站面板
 */
define(["jquery", "durandal/app", "durandal/composition", "knockout", "common", "http", "panal", "pager", "echarts", "slickGrid", "watertype", "mapUtils", "jqxgrid.selection", "WdatePicker", "bootstrap","jqxgrid.columnsresize"],
    function ($, app, composition, ko, common, http, panal, pager, echarts, Slick, w, mapUtils) {
        ...
        var rivers = {
            init: function () {
                var that = this;
                composition.addBindingHandler("strbprpzzInitHandler", {
                    init: function (dom) {
            ...
            initLayout: function () {
                ...
                // var _normalColumns = that._columns.slice(0, 2);
                // _normalColumns.push(that._columns.slice(24, 25)[0]);
                // _normalColumns.push(that._columns.slice(16, 17)[0]);

                modal.IsMax(panalObj.getSizeState() == "max");
                var jqHeight = $(".app-panal-content").height() - 27 * (panalObj.getSizeState() == "max" ? 4 : 5) - 50;
                var columns = panalObj.getSizeState() == "max" ? that._columns :  that._normalColumns;  // _normalColumns;  // that._columns.slice(0, 2)
                var datadatafields = [];
		...
		metadata = _metadata;   //  保存源数据
                    $('#strbprpzzList').on('rowclick', function (event) {
            ...
            getListData: function (currentPage) {
                ...
                if (panalObj.getSizeState() == "max") {
                    requestParams.sttp = "ZZ";
                    requestParams.usfl = 0;

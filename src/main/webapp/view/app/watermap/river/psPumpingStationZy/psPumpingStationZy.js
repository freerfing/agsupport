...
define(["jquery", "durandal/app", "durandal/composition", "knockout", "common", "http", "panal", "pager", "echarts", "slickGrid", "watertype", "mapUtils", "jqxgrid.selection", "WdatePicker", "bootstrap"],
    function ($, app, composition, ko, common, http, panal, pager, echarts, Slick, w, mapUtils) {
        ...
        var rivers = {
            init: function () {
                var that = this;
                composition.addBindingHandler("psPumpingStationZyInitHandler", {
                    init: function (dom) {
            ...
            initLayout: function () {
                ...
                $("#psPumpingStationZyList").jqxGrid({
                    width: '100%',
                    height: jqHeight,
                    source: dataAdapter,
                    rowsheight: 25,
            ...
            initTableColumns: function () {
                var that = this;
                w.queryMetadata("PS_PUMPING_STATION_ZY", function (columnsInfo, prikey, _metadata) {
                    columnsInfo[0].width = "180";
                    columnsInfo[1].width = "130";
                    that._columns = columnsInfo;
                    that.initLayout();
                    metadata = _metadata;   //  保存源数据
                    $('#psPumpingStationZyList').on('rowclick', function (event) {
                        ...
                        var layer = {
                            ...
                            tableName: "PS_PUMPING_STATION_ZY",
                            params: {
                                "prikey": prikey,
                                "row": dataObj,
                            },
            ...
            getListData: function (currentPage) {
                ...
                var url = auGurit.global.rootPath + "/subject/listpspumpingstationzy";
                $.ajax({
                    type: 'POST',
                ...
                if (that._data && that._pageInfo.totalPage >= 1) {
                    ...
                    if (!that._pager) {
                        that._pager = pager.getInstance({
                            parent: $("#psPumpingStationZyPager"),

...
define(["jquery", "durandal/app", "durandal/composition", "knockout", "common", "http", "panal", "pager", "echarts", "slickGrid", "watertype", "mapUtils", "jqxgrid.selection", "WdatePicker", "bootstrap"],
    function ($, app, composition, ko, common, http, panal, pager, echarts, Slick, w, mapUtils) {
        ...
        var rivers = {
            init: function () {
                var that = this;
                composition.addBindingHandler("wqsinffqInitHandler", {
                    init: function (dom) {
            ...
            initLayout: function () {
                ...
                var jqHeight = $(".app-panal-content").height() - 27 * (panalObj.getSizeState() == "max" ? 4 : 5) - 60;
            ...
            initTableColumns: function () {
                var that = this;
                w.queryMetadata("WQ_WQSINF_B", function (columnsInfo, prikey, _metadata) {
                    var normalColumns = [];
                    normalColumns.push({
                        align:"center",
                        cellsalign:"center",
		 ...
		 metadata = _metadata;   //  保存源数据
                    $('#wqsinffqList').on('rowclick', function (event) {
            ...
            getListData: function (currentPage) {
                ...
                var url = auGurit.global.rootPath + "/subject/listwqsinfb";
                $.ajax({
                    type: 'POST',

define(["jquery", "durandal/app", "durandal/composition", "knockout", "common", "http", "panal","panalPop", "echarts3_8_4", "tabs", "WdatePicker", "layer"],
    function ($, app, composition, ko, common, http, panal,panalPop, echarts, tabs, wdatePicker, layer) {
        ...
        var List = {
            init: function () {
                var that = this;
                composition.addBindingHandler("xgasWinInitHandler", {
                    init: function (dom) {
            ...
            loadTabs: function () {
                ...
                var t = tabs.getInstance([{
                    id: "xgasInfo",
                    title: "气体监测详情",
                    content: '<div id="xgas_detail" class="detail_tab_container"></div>',
                    selected: true
                }], {
                    replace: $("#xgasWinId"),
                    contentBorder: true,
            ...
            loadData: function () {
                ...
                var content = String()
                    + '<div style="margin-top:15px; width:100%; text-align: left;">'
                        ...
                        + that.constructBasicInfoRow(['站名：', '设备ID：'], [rowData.sname, rowData.devid])
                        + that.constructBasicInfoRow(['氧气（%）：', '一氧化碳（ppm）：'], [rowData.o2, rowData.co])
                        + that.constructBasicInfoRow(['硫化氢（ppm）：', '氨气（ppm）：'], [rowData.h2s, rowData.nh3])
                        + that.constructBasicInfoRow(['甲烷（%LEL）：', '电池电压（V）：'], [rowData.ch4, rowData.bat])
                        + that.constructBasicInfoRow(['采集时间：'], [rowData.tm])
                    + '</div>';

                $("#xgas_detail").append(content);
            },
            getData: function () {
                var that = this;
